...
import { collection, collectionGroup, query, where, getDocs, doc, updateDoc , onSnapshot} from 'firebase/firestore';
import { db } from '@/firebaseConfig';
interface UserProfile {id: string;firstName: string;lastName: string;userMobile: string;role: string;status: string;collection: string;}
const AlertTab = ({ label, count, color, icon, onPress }: any) => (
    <TouchableOpacity style={[styles.alertTab, { backgroundColor: color }]} onPress={onPress} activeOpacity={0.7}>
...
export default function Dashboard() {
    ...
    const motherStats = {weeksPregnant: 24, nextCheckup: "March 15, 2026"};
    ...
    const filteredUsers = pendingUsers.filter(user => {const query = searchQuery.toLowerCase();const firstName = (user.firstName || "").toLowerCase();const lastName = (user.lastName || "").toLowerCase();const mobile = (user.userMobile || "");return (firstName.includes(query) || lastName.includes(query) || mobile.includes(query));});
    useEffect(() => {
        if (role) {
            const currentRole = String(role);setUserRole(currentRole);if (currentRole === "Admin" || currentRole === "Supervisor") {fetchPendingUsers();
            } else {setLoading(false);}
        }
    }, [role]);
    const fetchPendingUsers = async () => {setLoading(true);
        const allPending: UserProfile[] = [];
        const collections = ["users", "beneficiaries"];
        try {
            for (const colName of collections) {const q = query(collection(db, colName), where("status", "==", "Pending"));
            ...
            console.log("Final State Array:", allPending);
        } catch (error) {
    ...
    useEffect(() => {
        ...
        if (userRole === "ASHA Worker") {fetchVisitMetrics();
        } else if (userRole === "Admin" || userRole === "Supervisor") {fetchPendingUsers();}
        else {setLoading(false);}
        }, [userMobile, userRole]);
            ...
            <View style={{flex: 1}}>
                ...
                <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
                    ...
                    {(userRole === "Supervisor" || userRole === "Admin") && (<View style={styles.adminSection}>
                            ...
                            <View style={styles.approvalListContainer}>{loading ? (<ActivityIndicator color="#1F7A6B" style={{margin: 20}}/>
                                ) : filteredUsers.length > 0 ? (filteredUsers.slice(0, 3).map((item) => (
...
const styles = StyleSheet.create({subText: {fontSize: 14, color: '#666', marginTop: 5,},
    ...
    iconBtn: { padding: 8, backgroundColor: '#f0f0f0', borderRadius: 8 },    alertLabel: { color: 'white', fontSize: 11, marginLeft: 4 },
